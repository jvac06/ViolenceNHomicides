...
cols <- c("relinquishment", "recordsall",                           "recordsallh",                          "recordsdealer",                       
 "recordsdealerh",                       "reportall" ,                           "reportallh"  ,                        
 "reportdealer" ,                        "reportdealerh"     ,                   "purge"             ,                  
 "registration",                         "registrationh"    ,                    "defactoreg" ,                         
 "defactoregh"  ,                        "UniversalBackgroundChecksAllFirearms", "UniversalBackgroundChecksHandguns"  , 
 "WaitingPeriodAllFirearms"     ,        "WaitingPeriodHandguns"       ,         "PermitAllFirearms"    ,               
 "PermitHandguns"      ,                 "PermitCarryingConcealedWeapons"   ,    "BanAssaultWeapons"     ,              
...
## Calculating Homicide Rate and violent crime rate per 100,000 people
```{r}
df$HomicideRate<-(df$NumHomicides/df$Population)*100000
df$CrimeRate<-(df$ViolentCrime/df$Population)*100000
glimpse(df)
```
```{r}
df$PopulationDensity<-df$Population/df$Area
df$Firearm<-df$TotalWeapons-df$Silencer4
```
...
df$FirearmProliferation<-df$Firearm/df$Population*1000
...
ggplot(df, aes(x=Black, y=HomicideRate)) +
  geom_point() +
  geom_smooth()+
  labs(title="Black vs. NumHomicides/Population")
...
```{r}
numerics=df[c('HomicideRate','CrimeRate','DrugUsers','AlcoholUsers','LawOfficers','LawCover','GDP','MentalIllness','WeaponProliferation','FirearmProliferation','GiniIndex','UnemploymentRate','Black','Hispanic','Adults 19-25','Education','GunLawStrictness','PopulationDensity')]
...
```{r}
reg1<-lm(formula = HomicideRate~MentalIllness+GDP+LawOfficers+LawCover+AlcoholUsers+DrugUsers+`Big Cities`+GunLawRank+Black+GiniIndex+Year+UnemploymentRate+WeaponProliferation+Education,data = df)
...
## OLS with log
```{r}
reg3<-lm(formula = log(HomicideRate)~MentalIllness+GDP+LawOfficers+LawCover+AlcoholUsers+DrugUsers+`Big Cities`+GunLawRank+Black+GiniIndex+Year+UnemploymentRate+WeaponProliferation+Education,data = df)
summary(reg3)
...
## LMER with FE (States)
```{r}
reg2<-lmer(formula = log(HomicideRate)~MentalIllness+GDP+LawOfficers+LawCover+AlcoholUsers+DrugUsers+df$`Big Cities`+GunLawRank+Region+Black+GiniIndex+(1|State)+(1|Year)+UnemploymentRate+Education+WeaponProliferation,data = df)
summary(reg2)
vif(reg2)
AIC(reg1)
AIC(reg2)
...
## Creating Lags
```{r}
df <- df %>% rename(ReportAllFirearms = reportall, 
                    GunRegistration = defactoreg,
