...
```{r}
ggplot(df, aes(x=df$HomicideRate)) + geom_histogram(color="gold",fill="seagreen")+ggtitle("Histogram of Homicide Rate")+xlab("Homicide Rate")

ggplot(df, aes(x=log(df$HomicideRate))) + geom_histogram(color="gold",fill="seagreen")+ggtitle("Histogram of Log Homicide Rate")+xlab("Log of Homicide Rate")
```
...
```{r}
reg1<-lm(formula = HomicideRate~df$MentalIllness+df$GDP+df$LawOfficers+df$LawCover+df$AlcoholUsers+df$DrugUsers+df$`Big Cities`+df$Education+df$GunLawRank+df$Black+df$GiniIndex+State+Year,data = df)
summary(reg1)
