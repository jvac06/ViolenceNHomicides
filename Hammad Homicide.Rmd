...
reg1<-lm(formula = HomicideRate~MentalIllness+GDP+LawOfficers+LawCover+AlcoholUsers+DrugUsers+`Big Cities`+Education+GunLawRank+Black+GiniIndex+Year,data = dff,na.action = na.exclude)
summary(reg1)
...
```{r}
reg2<-lmer(formula = HomicideRate~df$MentalIllness+df$GDP+df$LawOfficers+df$LawCover+df$AlcoholUsers+df$DrugUsers+df$`Big Cities`+df$Education+df$GunLawRank+df$Black+df$GiniIndex+(1|State)+Year,data = df)
summary(reg2)
...
```{r}
reg3<-lm(formula = HomicideRate~MentalIllness+GDP+LawOfficers+LawCover+AlcoholUsers+DrugUsers+`Big Cities`+Education+GunLawRank+Black+GiniIndex+Year,data = Y18,na.action = na.exclude)
summary(reg3)
```